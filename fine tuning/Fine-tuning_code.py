...
# Use a single GPU by default (set to "0"; adjust if needed)
...
# -----------------------------
# Custom BERT (compatible with pretrained structure)
# -----------------------------
...
class CustomBertSelfAttention(BertSelfAttention):
    ...
    def forward(
        ...
        # Add a learnable scaled bias (e.g., correlation prior) aligned to the current chunk length
...
class ProteinBERTModel(nn.Module):
    ...
    def forward(self, expressions, output_attentions=False):
        ...
        for i in range(total_chunks):
            ...
            seq_lengths.append(chunk_len + 1)
        ...
        if output_attentions:
            ...
            for attentions, start, end in attention_list:
                ...
                for layer_attn in attentions:
                    ...
                    cls_attn = layer_attn[0, :, 0, 1:]  # exclude CLS->CLS
                    cls_attn_mean = cls_attn.mean(dim=0)  # [seq_len-1]
                    cls_attentions.append(cls_attn_mean)

                cls_attentions = torch.stack(cls_attentions, dim=0).mean(dim=0)  # [seq_len-1]
...
class ProteinClassificationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(expr), label, eid
...
def evaluate(
    ...
    with torch.no_grad():
        for expressions, y, eids in tqdm(loader, desc=f"Evaluating [{dataset_type}]", leave=False):
    ...
    recall_val = recall_score(trues, preds, average="binary", zero_division=0)
...
def train_classifier_epoch(
    ...
    train_results = {"trues": trues, "preds": preds, "probs": y_scores, "eids": eids_list}
...
def visualize_attention(
    ...
    attention_df = pd.DataFrame({"Protein": protein_names, "Attention": aggregate_attention})
...
if __name__ == "__main__":
    ...
    expression_data = pd.read_csv("imputed_data.csv", dtype={"eid": str})
    expression_data.set_index("eid", inplace=True)

    with np.load("global_representations.npz") as data:
        protein_features = {key: data[key] for key in data.files}

    protein_correlation_matrix = pd.read_csv("protein_correlation_matrix.csv", index_col=0)
    ...
    label_files = [f for f in os.listdir(".") if f.endswith("_labels.csv")]
    disease_eids_dict = extract_disease_labels(label_files, expression_data.index)

    healthy_df = pd.read_csv("healthy_eids_test.csv", dtype={"eid": str})
    healthy_eids = set(healthy_df["eid"].tolist()) & set(expression_data.index)

    protein_names = common_proteins

    unfrozen_layers_list = [8]

    for num_unfrozen_layers in unfrozen_layers_list:
        logging.info("\n" + "=" * 50)
        logging.info(f"Start: unfreeze {num_unfrozen_layers} layers")
        logging.info("=" * 50)

        results_dir = f"unfreeze_{num_unfrozen_layers}_layers"
        ...
        for disease_name, disease_eids in disease_eids_dict.items():
            logging.info(f"Processing disease: {disease_name} (unfreeze {num_unfrozen_layers} layers)")

            current_eids = list(disease_eids | healthy_eids)
            eid_labels = [1 if eid in disease_eids else 0 for eid in current_eids]

            if len(current_eids) == 0:
                logging.warning(f"No samples for disease {disease_name}; skipping.")
                continue
            ...
            logging.info(f"Train size: {len(train_eids)}, Test size: {len(test_eids)}")
            ...
            train_loader = DataLoader(
                train_dataset, batch_size=batch_size, shuffle=True, num_workers=25, pin_memory=True
            )
            test_loader = DataLoader(
                test_dataset, batch_size=100, shuffle=False, num_workers=25, pin_memory=True
            )
            ...
            base_model.load_state_dict(torch.load("protein_bert_model_final.pt", map_location="cpu"), strict=False)
            ...
            for layer in model_for_freeze.bert.encoder.layer[-num_unfrozen_layers:]:
            ...
            optimizer = torch.optim.Adam(
                ...
                + list(filter(lambda p: p.requires_grad, model_for_freeze.parameters())),
                lr=5e-5,
            )
            ...
            for epoch in range(epochs):
                logging.info(f"Training Epoch {epoch + 1}/{epochs} (unfreeze {num_unfrozen_layers} layers)")
                ...
                recall_val = test_results["recall"]
                ...
                if not np.isnan(auc_score) and auc_score > best_auc:
                    ...
                    if isinstance(base_model, nn.DataParallel):
                        torch.save(
                            ...
                            os.path.join(results_dir, f"best_base_model_{disease_name}.pt"),
                        )
                        torch.save(
                            classifier_head.module.state_dict(),
                            os.path.join(results_dir, f"best_classifier_head_{disease_name}.pt"),
                        )
                    else:
                        torch.save(
            ...
            if best_test_results is None:
                logging.warning(
                    f"{disease_name}: AUC was never computed across epochs (likely only one class in test set)."
                )
                continue

            logging.info(f"{disease_name} best Test AUC={best_auc:.4f} (unfreeze {num_unfrozen_layers} layers)")
            ...
            all_disease_metrics.append(
                {
                    ...
                    "Test_Recall": best_test_results["recall"],
            ...
            pred_out_path = os.path.join(results_dir, f"predicted_probabilities_{disease_name}.csv")
            df_preds.to_csv(pred_out_path, index=False)
            logging.info(f"Saved predictions to {pred_out_path}")

            logging.info(f"Visualizing attention for {disease_name} (unfreeze {num_unfrozen_layers} layers)")
            visualize_loader = DataLoader(
                test_dataset, batch_size=10, shuffle=False, num_workers=10, pin_memory=True
            )
            ...
            with torch.no_grad():
                for expressions_vis, y_vis, eids_vis in tqdm(
                    visualize_loader, desc=f"Attention visualization for {disease_name}", leave=False
                ):
                    expressions_vis = expressions_vis.to(device)

                    if isinstance(base_model, nn.DataParallel):
                        cls_emb_vis, attentions_vis = base_model.module.forward(
                            expressions_vis, output_attentions=True
                        )
                    else:
            ...
            heatmap_path = os.path.join(results_dir, f"attention_heatmap_{disease_name}.pdf")
            barplot_path = os.path.join(results_dir, f"attention_barplot_{disease_name}_top10.pdf")
        ...
        if all_disease_metrics:
            ...
            summary_path = os.path.join(results_dir, "disease_model_metrics_summary.csv")
            summary_df.to_csv(summary_path, index=False)
            logging.info(f"Saved summary metrics to {summary_path}")

        logging.info(f"Finished all diseases for unfreeze={num_unfrozen_layers}.")

    logging.info("All unfreeze configurations are completed.")